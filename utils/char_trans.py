...
class char_trans(Fourier):
    ...
    def write_spline_dot(self,img_path=None,X=None,Y=None):
        img = np.zeros((self.width,self.height),np.uint8)
    ...
    def write_fourier_dot(self,img_path=None,X=None,Y=None):
        X, Y, _ = self.write_spline_dot(img_path,X,Y)
